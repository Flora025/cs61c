...
Color *evaluateOneCell(Image *image, int row, int col, uint32_t rule)
{
	...
	for (int i = 0; i < 8; ++i)
	{
		...
		int index = curY * image->cols;		   // index of curcolor
...
Image *life(Image *image, uint32_t rule)
{
	...
	for (int r = 0; r < image->rows; r++)
	{
		...
		{
			decoImg->image[c + r * image->cols] = evaluateOneCell(image, r, c, rule);
		}
	}
...
You may find it useful to copy the code from steganography.c, to start. <- the course even teaches cv magic lol
*/
